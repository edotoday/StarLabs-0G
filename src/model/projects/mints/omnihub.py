...
class OmniHub:
    ...
    async def mint(self):
        try:
            ...
            contracts_to_mint = await self._get_random_token_for_mint(max_price_to_mint)

            if not contracts_to_mint:
                logger.error(f"{self.account_index} | No contracts to mint found")
                return False
    ...
    async def _mint_nft(self, contract: dict):
        try:
            ...
            # Add a small buffer to ensure enough value
            value_with_buffer = (
                float(price) * random.uniform(1.01, 1.03)
            )  # Convert to float first, then add 1% buffer

            tx_hash = await self.web3.send_transaction(
                to=address,
                data=payload,
                wallet=self.wallet,
                value=self.web3.web3.to_wei(value_with_buffer, "ether"),
                chain_id=CHAIN_ID,
            )
            ...
            if receipt["status"] == 1:
                logger.success(
                    f"{self.account_index} | Successfully minted OmniHub NFT at {title}: {EXPLORER_URL_0G}{tx_hash}"
                )
                return True
            else:
                logger.error(
                    f"{self.account_index} | Transaction failed: {EXPLORER_URL_0G}{tx_hash}"
                )
                return False
            ...
            for retry in range(3):
                try:
                    ...
                    response = await self.session.get(
                        "https://api.omnihub.xyz/api/contract/list",
                        params=params,
                        headers=headers,
                    )
                    ...
                    for contract in data:
                        price_eth = self.web3.web3.from_wei(
                            int(contract["fee"]), "ether"
                        )
