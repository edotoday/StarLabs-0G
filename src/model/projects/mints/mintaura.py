...
async def mintaura_pandriel(
    ...
    try:
        PANDRIEL_0G_CONTRACT = "0x6f0c8e8c6674a72388a1ca4f1eb805dc58b8ab09"
        logger.info(f"{account_index} | Checking balance of Pandriel 0G NFT...")

        # Проверяем баланс NFT на кошельке
        nft_contract = web3.web3.eth.contract(
            address=web3.web3.to_checksum_address(PANDRIEL_0G_CONTRACT), abi=NFT_ABI
        )
        ...
        logger.info(f"{account_index} | Starting mint of Pandriel 0G NFT...")
        ...
        tx_params = {
            "from": wallet.address,
            "to": web3.web3.to_checksum_address(PANDRIEL_0G_CONTRACT),
            "value": 0,  # Минт бесплатный
            "data": data,
            "nonce": await web3.web3.eth.get_transaction_count(wallet.address),
            "chainId": CHAIN_ID,
        ...
        if tx_hash:
            logger.success(f"{account_index} | Successfully minted Pandriel 0G NFT")
            return True

        raise Exception("Failed to mint Pandriel 0G NFT")
        ...
        logger.error(
            f"{account_index} | Error minting Pandriel 0G NFT: {e}. Waiting {random_pause} seconds..."
        )
        await asyncio.sleep(random_pause)
        raise
